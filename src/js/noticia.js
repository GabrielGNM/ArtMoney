    ...
    if (noticias) {
        for (var index = 0; index < noticias.length; index++) {
            ...
            newDiv.innerHTML = `
                ...
                <button onClick="ComentarioBtn(this)">Comentários</button>
                `;
...
function ComentarioBtn(clickedButton) {
    var parentDiv = clickedButton.parentNode;
    var h2Element = parentDiv.querySelector('h2');
    var h2Text = h2Element.textContent;
    var sessionUser = {
        TituloDoComentario: h2Text,
    };
    var userJSON = JSON.stringify(sessionUser);
    sessionStorage.setItem('TituloDoComentario', userJSON);
